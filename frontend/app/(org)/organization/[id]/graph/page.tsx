...
function GraphInner({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Use the actual container size for layout — fall back to sensible defaults
      const w = window.innerWidth;
      const h = window.innerHeight;
      const { rfNodes, rfEdges } = computeLayout(data, w, h);
      ...
      setStats({
        ...
        types: new Set(visibleNodes.map((n) => n.data.resourceType)).size,
      });
  ...
  return (
    ...
    >
      ...
      {/* Canvas — key fix: use flex-1 with min-h-0 so it doesn't overflow the screen */}
      <div
        className="flex-1 relative bg-[#07070f] min-h-0"
        style={{ height: "100%" }}
      >
        {error && (
          <div
        ...
        {loading && (
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-[#07070f]/80 backdrop-blur-sm">
        ...
        <ReactFlow
          ...
          onInit={(instance) => {
            setTimeout(
              () => instance.fitView({ padding: 0.14, duration: 600 }),
              50,
            );
          }}
        >
...
export default function RelationshipGraphPage() {
  ...
  useEffect(() => {
    ...
    graph
      ...
      .then((data: ResourceRelationship[]) => {
        ...
        setGraph_ResourceRows(
          Array.from(nodeMap.values()).sort(
  ...
  return (
    <div className="h-screen bg-[#07070f] overflow-hidden flex flex-col">
      {view === "list" && (
        <ResourceList
